...
| reviewer | nothing — holds `Bash`; write-boundary best-effort, outputs gated | everything | An agent that can act on its own finding cannot be trusted to report one it would rather not act on. |
...
| auditor | nothing — holds `Bash`; write-boundary best-effort, outputs gated | everything | The trivial fix is how an auditor becomes an editor of the records it audits. |
