...
check 4 fails a change under `src/` that leaves it stale. `DESIGN.md` is
writable by **nobody** — amendments are §6 and human.
...
2. **`hooks/boundary.sh`** at `PreToolUse` — a *path* boundary. `tools:` cannot
...
cannot deny a tool the agent was never given. See ADR-0007 and ADR-0013.

**Known limit, stated so it is not mistaken for coverage:** the shell check is
